...
// Middleware
app.use(bodyParser.json());
app.use(express.static(path.join(__dirname, 'public'), { index: false })); // Disable default index serving
app.use(express.urlencoded({ extended: true }));
app.use(session({
    secret: process.env.SESSION_SECRET,
    resave: false,
...
// Routes
app.get('/', (req, res) => {
    // Always serve login.html at root, even if authenticated
    res.sendFile(path.join(__dirname, 'public', 'login.html'));
});
...
app.get('/exam', isAuthenticated, (req, res) => {
    res.sendFile(path.join(__dirname, 'public', 'index.html'));
});
